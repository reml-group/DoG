...
if __name__ =="__main__":
    ...
    for q in json_data:
        ...
        write_to_file(output_path,f"question_id: {q['QuestionId']}")
...
A:"""
            ...
            if matches is None:
                ...
                for match in matches:
                    try:
                        best_rel_list.append(mapping_rel_dict[match])
                    except:
                        error = 1
                        break
            if not best_rel_list:
...
Knowledge Triplets:{output_str}"""
        ...
        answer_list = []
        for parser in q["Parses"]:
            for item in parser["Answers"]:
                answer_list.append(item["EntityName"])
        find = 0
        if not need_LLM:
            try:
        ...
        for answer in answer_list:
            item_processed = answer.replace(" ", "").lower()
