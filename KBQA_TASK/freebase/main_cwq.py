...
if __name__ =="__main__":
    ...
    for q in json_data:
        ...
        need_LLM = 0
        try:
            start_entity = list(q["topic_entity"].items())[0] # dict
        except:
            continue
...
Knowledge Triplets:{output_str}"""
        ...
        answer = q["answer"]
        processed_answer = answer.lower().replace(" ", "")
        find = 0
        if not need_LLM:
            try:
                generated_answer_processed = generated_answer.replace(" ", "").lower()
                if processed_answer in generated_answer_processed:
                    find = 1
            except:
                find = 0
